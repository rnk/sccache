...
use crate::{
    config::{ReapiConfig, ReapiToolchainMode},
    dist::{Client as _, CompileCommand, PathTransformer, RunJobResult, Toolchain, pkg},
};
...
/// Build the tar that `InputsPackager::write_inputs` would have produced.
fn inputs_tar(entries: &[(&str, &[u8], u32)]) -> std::fs::File {
    let file = tempfile::tempfile().unwrap();
    // Plain, uncompressed: `InputsPackager::write_inputs` writes a bare tar
    // and leaves compression to whoever is putting it on the wire.
    let mut builder = tar::Builder::new(file.try_clone().unwrap());
...
/// Hands the client inputs the test already has, in whichever of the two
/// representations is under test.
struct TestPackager {
    entries: Option<Vec<pkg::InputEntry>>,
    archive: Option<std::fs::File>,
}

impl pkg::InputsPackager for TestPackager {
    fn write_inputs(
        self: Box<Self>,
        wtr: &mut dyn std::io::Write,
    ) -> anyhow::Result<PathTransformer> {
        let mut archive = self.archive.expect("no archive to write");
        std::io::copy(&mut archive, wtr)?;
    ...
    fn list_inputs(self: Box<Self>) -> anyhow::Result<(Vec<pkg::InputEntry>, PathTransformer)> {
        Ok((
            self.entries.expect("no entries to list"),
            PathTransformer::new(),
        ))
    }
}
...
async fn run_one(fixture: &Fixture, source: &[u8]) -> anyhow::Result<RunJobResult> {
...
/// Drive one job through the upstream `dist::Client` flow.
async fn run_with(fixture: &Fixture, packager: TestPackager) -> anyhow::Result<RunJobResult> {
    let toolchain = Toolchain {
        archive_id: "test-toolchain".to_owned(),
    };
    let alloc = fixture
        .client
        .do_alloc_job(toolchain.clone())
        .await
        .unwrap();
    let crate::dist::AllocJobResult::Success { job_alloc, .. } = alloc else {
        panic!("allocation failed");
    };

    // The compiler travelled with the inputs, so there is nothing for
    // `do_submit_toolchain` to upload.
    fixture
        .client
        .toolchains
        .lock()
...
async fn entries_and_archive_agree_on_the_input_root() {
    ...
    for response in [from_archive, from_entries] {
        let RunJobResult::Complete(result) = response.unwrap() else {
            panic!("expected a completed job");
        };
        assert_eq!(result.output.code, 0);
    }
}
...
async fn repeated_inputs_are_hashed_once() {
    ...
    run_one_from_entries(&fixture, &entries).await.unwrap();
...
async fn compiles_remotely_end_to_end() {
    ...
    let fixture = fixture(ReapiToolchainMode::Inputs).await;
    let response = run_one(&fixture, b"the source text").await.unwrap();

    let RunJobResult::Complete(result) = response else {
        panic!("expected a completed job");
    };
...
async fn identical_work_produces_one_action_digest() {
    ...
    run_one(&fixture, b"same source").await.unwrap();
    run_one(&fixture, b"same source").await.unwrap();
    run_one(&fixture, b"different source").await.unwrap();
...
async fn blobs_are_only_uploaded_once() {
    ...
    run_one(&fixture, b"some source").await.unwrap();
...
async fn large_inputs_take_the_streaming_path() {
    ...
    let fixture = fixture(ReapiToolchainMode::Inputs).await;
    let response = run_one(&fixture, &source).await.unwrap();
...
async fn compiler_failure_is_reported_as_a_completed_job() {
    ...
    assert_eq!(result.output.code, 1);
    assert_eq!(
        String::from_utf8_lossy(&result.output.stderr),
        "error: no\n"
    );
...
#[tokio::test]
async fn a_job_whose_toolchain_was_never_uploaded_fails_loudly() {
    // `do_submit_toolchain` is what puts the compiler in the CAS. If it never
    // ran, the action would reference a compiler the worker cannot see, and
    // failing here is much easier to diagnose than a remote "no such file".
    let fixture = fixture(ReapiToolchainMode::Inputs).await;
    let alloc = fixture
        .client
        .do_alloc_job(Toolchain {
            archive_id: "never-uploaded".to_owned(),
        })
        .await
        .unwrap();
    let crate::dist::AllocJobResult::Success { job_alloc, .. } = alloc else {
    ...
    let err = fixture
        ...
        .do_run_job(
            ...
            compile_command(),
            vec!["/work/proj/build/a.o".to_owned()],
            Box::new(TestPackager {
                entries: None,
                archive: Some(inputs_tar(&[(
    ...
    assert!(
        err.to_string().contains("never uploaded"),
...
async fn image_mode_sends_no_toolchain() {
    ...
    // Nothing to upload, so sccache never calls do_submit_toolchain.
    let crate::dist::AllocJobResult::Success { need_toolchain, .. } = alloc else {
        panic!("allocation failed");
    };
